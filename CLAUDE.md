...
node --env-file=.env.local scripts/frames-to-blob.mjs  # upload public/frames to Blob (resumable) — run after any frame/MP4 script
```
...
- Project pages: `src/components/scroll/ProjectStory.tsx` + `src/lib/project-story.ts` — per-slug walkthroughs with copy on the left and facts on the right. Add a `projectStories` entry only once its frames are uploaded to Blob — nothing checks that they exist. Server code must never touch `public/frames` or `public/images` through `fs`: Next then traces every frame into each function bundle, which is what filled the 10 GB Vercel "Functions Storage" quota. `outputFileTracingExcludes` in `next.config.ts` guards against it.
...
- The built-in frames (~265 MB) were removed from the repo on 2026-09-26 but remain in its git history, so clones stay large until the history is rewritten (a separate decision; it changes every commit hash).

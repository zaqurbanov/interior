...
- `public/frames` (~265 MB) is committed. Moving it to Vercel Blob needs a paid plan (Hobby: 2,000 uploads a month, 10k reads); `scripts/frames-to-blob.mjs` and `NEXT_PUBLIC_FRAMES_URL` are ready for that. Admin "Make frames" jobs also upload hundreds of files to Blob per run, so on Hobby they hit the same limit.

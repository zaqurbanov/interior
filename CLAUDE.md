...
node --env-file=.env.local scripts/frames-to-blob.mjs  # copy public/frames to Blob (resumable) — not used now, see Known gaps
```
...
- Project pages: `src/components/scroll/ProjectStory.tsx` + `src/lib/project-story.ts` — per-slug walkthroughs with copy on the left and facts on the right. Add a `projectStories` entry only together with its frames in `public/frames` — nothing checks that they exist. Server code must never touch `public/frames` or `public/images` through `fs`: Next then traces every frame into each function bundle, which is what filled the 10 GB Vercel "Functions Storage" quota. `outputFileTracingExcludes` in `next.config.ts` guards against it.
...
New walkthroughs are made on a computer, not in the admin: video in `videos/`, an entry in `scripts/extract-project-frames.mjs`, run it, then a `projectStories` entry — step by step in `docs/animasiya-yaratmaq.md` (the studio does this itself). The admin only edits existing ones (`/admin/projects/<id>/story`, `Story` model): copy, stage positions, the hold and visibility. `getStory()` in `data.ts` prefers an enabled, framed `Story` document and otherwise falls back to the built-in `projectStories` entry (whose frames must be in `public/frames`). Stories carry a `base`: `/frames` for built-in frames, or the Blob URL prefix for generated ones — all frame, poster and MP4 URLs go through the helpers in `project-story.ts` (home: `framesUrl()` in `src/lib/frames-base.ts`), never hand-built paths. `/frames` means "the built-in frames, wherever `FRAMES_BASE` serves them" — `public/frames` in the repo, served as Vercel static files (`NEXT_PUBLIC_FRAMES_URL` can point them at a CDN later; the old `NEXT_PUBLIC_FRAMES_BASE` is deliberately ignored). They were briefly moved to Vercel Blob, but the Hobby plan allows 2,000 uploads ("advanced operations") a month and 6,294 frames blocked the store — keep bulk frames out of Blob on that plan. "Edit" on a built-in walkthrough copies it into the database (same frames, same version); "Discard edits" returns to the code version. Admin frame generation (video upload, GitHub Actions job, `/api/story-job`) was removed on 2026-09-26 at the studio's request; the `Story` model still has its `sources`/`job` fields for old records.
...
Enquiry paths: project pages end with `ProjectEnquiry` ("Planning something like …?" → `/contact?project=<title>`, which `ContactForm` uses to start the message, plus WhatsApp / phone / email). On phones `MobileContactBar` (site layout) pins Enquire · WhatsApp · Call to the bottom once the visitor has scrolled, and hides while a `[data-walkthrough]` section, `#contact` or the footer is on screen and on `/contact`. From `md` up, `WhatsAppButton` (site layout) is a round, pulsing WhatsApp button fixed bottom-right (pulse off under reduced motion). WhatsApp uses Site content → Contact → WhatsApp (`contact.whatsapp`), falling back to the studio phone (`whatsappNumber()`); links go through `telHref()` / `whatsappHref()` in `src/lib/contact-links.ts`. Service pages list example projects (`getProjectsForService()`): projects with services ticked in the admin (`Project.services`), otherwise matched by category (`projectServices()` in `data.ts`).
...
- `public/frames` (~265 MB) is committed. Moving it to Vercel Blob needs a paid plan (Hobby: 2,000 uploads a month, 10k reads); `scripts/frames-to-blob.mjs` and `NEXT_PUBLIC_FRAMES_URL` are ready for that.

...
- **Yeni animasiyanı studiya özü kompüterdə yaradır** (2026-09-26 qərarı): admindəki "Make frames" çıxarıldı. Addımlar: [animasiya-yaratmaq.md](animasiya-yaratmaq.md).
- **Deploy-dan əvvəl `npm run build`** — dev server dayandırılmış halda.
- **Kadrlar repodadır (`public/frames`), Blob-da yox.** 2026-09-26-da Blob-a köçürüldü, amma Vercel Hobby planında Blob ayda cəmi 2 000 yükləməyə ("Advanced Operations") icazə verir; 6 294 kadr limiti keçdi, store bloklandı və animasiyalar saytda itdi. Kadrlar repoya qaytarıldı. Blob-a yalnız pullu planda köçürmək olar (`scripts/frames-to-blob.mjs` + `NEXT_PUBLIC_FRAMES_URL`).

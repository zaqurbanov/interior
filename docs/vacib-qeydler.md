...
- **Kadrlar Blob-dadır, repoda yox** (2026-09-26-dan). `public/frames` yalnız lokal iş qovluğudur (git-ə düşmür). Lokal skriptlə (`extract-project-frames.mjs`, `extract-frames.mjs`, `build-mobile-videos.mjs`) yeni kadr çıxaranda sonra mütləq `node --env-file=.env.local scripts/frames-to-blob.mjs` işlət, yoxsa sayt yeni faylları tapmır. Yükləmədən əvvəl lokal baxmaq üçün `.env.local`-da `NEXT_PUBLIC_FRAMES_BASE=/frames`. Admin paneldən yaradılan animasiyalar buna ehtiyac duymur.
